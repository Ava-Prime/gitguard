...
class TestOPAServerIntegration:
    ...
    def opa_server(self):
        ...
        try:
            result = subprocess.run(["opa", "version"], capture_output=True)
            if result.returncode != 0:
    ...
    @pytest.mark.skipif(not Path("policies/guard_rules.rego").exists(), 
                       reason="guard_rules.rego not found")
...
class TestOPAPolicyFiles:
    ...
    def test_policy_test_coverage(self):
        ...
        policy_bases = {f.stem for f in policy_files if not f.stem.endswith("_test") and not f.stem.endswith("test")}
        test_bases = set()
    ...
    def test_policy_package_declarations(self):
        ...
        for policy_file in policy_files:
            ...
            assert package_line.startswith('package '), f"Invalid package declaration in {policy_file}"
...
class TestOPAPolicyWorkflows:
    ...
    def test_pr_merge_workflow(self):
        ...
        final_decision = (
            results["allow"]["result"] and 
            len(results["deny"]["result"]) == 0 and
            not results["exceeds_budgets"]["result"]
        )
        ...
        decision_record = {
            ...
            "policies_evaluated": [
                ...
                },
                {
                    "policy": "deny",
                    "result": False,
                    "explanation": "No denial conditions met"
                },
                {
            ...
            "input_data": {
                "pr": {"risk_score": 0.25, "checks_passed": True},
                "actor": "developer"
            }
        }
