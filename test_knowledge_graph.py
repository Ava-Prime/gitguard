...
try:
    ...
    class MockActivities:
        ...
        def _ensure_schema(conn):
            with conn.cursor() as cur:
                cur.execute("CREATE TABLE IF NOT EXISTS codex_nodes (id SERIAL PRIMARY KEY, ntype TEXT, nkey TEXT, title TEXT, data JSONB)")
                cur.execute("CREATE TABLE IF NOT EXISTS codex_edges (src INT, dst INT, rel TEXT, data JSONB)")
        ...
        def _mark_delivery_seen(conn, delivery_id: str):
            with conn.cursor() as cur:
                cur.execute("INSERT INTO codex_seen_deliveries (delivery_id) VALUES (%s) ON CONFLICT DO NOTHING", (delivery_id,))
        ...
        def _validate_vector_dimension(vector, expected_dim=1536):
            if len(vector) != expected_dim:
                raise ValueError(f"Vector dimension {len(vector)} does not match expected {expected_dim}")
            return True
        
        @staticmethod
        def _insert_embedding(conn, node_id: int, embedding: list):
            with conn.cursor() as cur:
                cur.execute("INSERT INTO codex_embeddings (node_id, embedding) VALUES (%s, %s) ON CONFLICT (node_id) DO UPDATE SET embedding = EXCLUDED.embedding", (node_id, embedding))
        
        @staticmethod
        def _cleanup_old_deliveries(conn, days_old=30):
            with conn.cursor() as cur:
                cur.execute("DELETE FROM codex_seen_deliveries WHERE created_at < NOW() - INTERVAL '%s days'", (days_old,))
                return cur.rowcount
...
class TestGraphAPIEndpoints(unittest.TestCase):
    ...
    def test_pr_graph_endpoint_success(self):
        ...
        connected_data = [
            ...
            {"id": "policy-1", "ntype": "Policy", "nkey": "policy:security-scan", "title": "security-scan"},
            {"id": "symbol-1", "ntype": "Symbol", "nkey": "symbol:src/main.py#main", "title": "main"}
        ]
