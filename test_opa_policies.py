...
class MockOPAClient:
        ...
        for path in changed_paths:
            if path.startswith("infra/") or path.startswith(".github/"):
                if "owner-approved" not in labels and "emergency-override" not in labels:
                    deny_reasons.append("Infrastructure change requires @platform-team approval")

            if path.startswith("policies/"):
                if "security-approved" not in labels and "emergency-override" not in labels:
                    deny_reasons.append("Policy change requires @security-team approval")

            if path.startswith("apps/guard-api/"):
                if "api-security-reviewed" not in labels and "emergency-override" not in labels:
                    deny_reasons.append("API change requires @backend-team security review")
    ...
    def _evaluate_exceeds_budgets(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if exceeds_coverage or exceeds_perf:
            ...
            return {"result": True, "explanation": "; ".join(reasons)}
...
class TestOPAPolicyEvaluation:
    ...
    def test_high_risk_area_by_score(self):
        ...
        input_data = {"pr": {"changed_paths": ["src/api.py"], "risk_score": 0.75}}
    ...
    def test_not_high_risk_area(self):
        ...
        input_data = {
            "pr": {"changed_paths": ["src/api.py", "tests/test_api.py"], "risk_score": 0.15}
        }
    ...
    def test_exceeds_budgets_coverage_threshold(self):
        ...
        input_data = {
            "pr": {"coverage_delta": -0.25, "perf_delta": 2.0, "development_phase": "production"},
            "repo": {"perf_budget": 5},
        }
    ...
    def test_exceeds_budgets_performance_threshold(self):
        ...
        input_data = {
            "pr": {"coverage_delta": 0.1, "perf_delta": 6.0, "development_phase": "feature_dev"},
            "repo": {"perf_budget": 5},
        }
    ...
    def test_within_budgets(self):
        ...
        input_data = {
            "pr": {"coverage_delta": 0.1, "perf_delta": 3.0, "development_phase": "feature_dev"},
            "repo": {"perf_budget": 5},
        }
    ...
    def test_development_phase_coverage_thresholds(self):
        ...
        for phase, threshold in phases_and_thresholds:
            ...
            ), f"Phase {phase} should pass with coverage {threshold + 0.01}"
            ...
            ), f"Phase {phase} should fail with coverage {threshold - 0.01}"
    ...
    def test_development_phase_exception_incomplete_tests(self):
        ...
        input_data = {
            "pr": {
                ...
                "file_analysis": {"test_api.py": {"incomplete_markers": 3}},
            },
            "repo": {"perf_budget": 5},
        }
...
class TestOPADenyPolicies:
    ...
    def test_deny_policy_change_without_approval(self):
        ...
        input_data = {
            "action": "merge_pr",
            "pr": {"labels": ["policy"], "changed_paths": ["policies/guard_rules.rego"]},
        }
    ...
    def test_deny_api_change_without_review(self):
        ...
        input_data = {
            "action": "merge_pr",
            "pr": {"labels": ["api"], "changed_paths": ["apps/guard-api/main.py"]},
        }
    ...
    def test_deny_dependencies_without_sbom(self):
        ...
        input_data = {
            "action": "merge_pr",
            "pr": {"labels": ["dependencies"], "changed_paths": ["requirements.txt"], "checks": {}},
        }
...
class TestOPAHelperFunctions:
    ...
    def test_dco_signoff_validation(self):
        ...
        for message in invalid_messages:
            has_proper_signoff = (
                "Signed-off-by:" in message and "<" in message and ">" in message and "@" in message
            )
