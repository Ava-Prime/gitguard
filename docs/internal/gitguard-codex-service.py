...
async def search_knowledge(q: str, entity_type: Optional[str] = None):
...
async def generate_health_recommendations(repo_name: str) -> List[str]:
    ...
    async with db_pool.acquire() as conn:
        ...
        if low_coverage_files:
            recommendations.append(f"Improve test coverage for {len(low_coverage_files)} files with <60% coverage")
        ...
        for pattern in risky_patterns:
            recommendations.append(f"Address recurring {pattern['pattern_type']} pattern ({pattern['occurrences']} recent occurrences)")
