...
MAX_API_KEY_ROTATION_DAYS = 365
MAX_SESSION_TIMEOUT_HOURS = 8
MIN_TLS_VERSION = 1.2

# Test expectation constants
EXPECTED_HIGH_CONFIDENCE_COUNT = 1
EXPECTED_MEDIUM_PLUS_COUNT = 2
EXPECTED_UNIQUE_FINDINGS_COUNT = 2
EXPECTED_CONFIG_LINE_NUMBER = 5
EXPECTED_FINDINGS_COUNT = 2
EXPECTED_TEST_FILE_LINE_NUMBER = 2
...
class TestSecurityScanning:
    ...
    @pytest.mark.asyncio
    async def test_run_security_scan_success(
        self, mock_subprocess_run, sample_bandit_output, sample_files_changed
    ):
        """Test successful security scan with bandit"""
        # Mock successful bandit execution
        mock_subprocess_run.return_value = Mock(
            stdout=json.dumps(sample_bandit_output), stderr="", returncode=0
        )
        ...
        assert len(findings) == EXPECTED_FINDINGS_COUNT
        ...
        assert findings[0]["line"] == EXPECTED_TEST_FILE_LINE_NUMBER
        ...
        assert findings[1]["line"] == EXPECTED_CONFIG_LINE_NUMBER
    ...
    @pytest.mark.asyncio
    async def test_run_security_scan_bandit_failure(
        self, mock_subprocess_run, sample_files_changed
    ):
    ...
    async def test_run_security_scan_invalid_json(self, mock_subprocess_run, sample_files_changed):
        ...
        mock_subprocess_run.return_value = Mock(
            stdout="invalid json output", stderr="", returncode=0
        )
...
class TestVulnerabilityDetection:
    ...
    def test_severity_classification(self):
        ...
        severity_map = {"HIGH": "critical", "MEDIUM": "high", "LOW": "medium", "INFO": "low"}
    ...
    def _map_severity(self, bandit_severity: str) -> str:
        ...
        mapping = {"HIGH": "critical", "MEDIUM": "high", "LOW": "medium", "INFO": "low"}
    ...
    def test_confidence_filtering(self):
        ...
        assert len(high_confidence) == EXPECTED_HIGH_CONFIDENCE_COUNT
        ...
        assert len(medium_plus) == EXPECTED_MEDIUM_PLUS_COUNT
    ...
    def test_vulnerability_deduplication(self):
        ...
        assert len(unique_findings) == EXPECTED_UNIQUE_FINDINGS_COUNT
...
class TestComplianceChecks:
    ...
    def test_security_policy_validation(self):
        ...
        findings = [{"test_id": "B105", "severity": "HIGH", "description": "Hardcoded password"}]
    ...
    def _check_security_compliance(self, findings: list[dict], policies: dict) -> list[dict]:
        ...
        if policies.get("block_hardcoded_secrets"):
            hardcoded_secrets = [
                f for f in findings if "hardcoded" in f.get("description", "").lower()
            ]
        ...
        for file_info in files:
            if file_info["path"].endswith(".key") and file_info["permissions"] != "600":
                ...
                if file_info["permissions"] not in ["600", "640"]:
                    violations.append(
                        f"Secret file {file_info['path']} has overly permissive permissions"
                    )
    ...
    def test_dependency_vulnerability_check(self):
        ...
        dependencies = [
            {"name": "requests", "version": "2.25.1", "vulnerabilities": []},
            {
                "name": "django",
                "version": "3.1.0",
                "vulnerabilities": [{"id": "CVE-2021-44420", "severity": "HIGH"}],
            },
            {
                "name": "numpy",
                "version": "1.19.0",
                "vulnerabilities": [{"id": "CVE-2021-33430", "severity": "MEDIUM"}],
            },
        ]
...
class TestSecurityConfiguration:
    ...
    def test_tls_configuration_validation(self):
        ...
        if float(tls_config["min_version"]) < MIN_TLS_VERSION:
    ...
    def test_authentication_configuration(self):
        ...
        if auth_config["session_timeout"] > MAX_SESSION_TIMEOUT_HOURS * 3600:
            violations.append("Session timeout is too long")

        # Check password policy
        pwd_policy = auth_config["password_policy"]
        if pwd_policy["min_length"] < MIN_PASSWORD_LENGTH:
    ...
    def test_api_security_configuration(self):
        ...
        api_config = {
            ...
            "api_key_rotation": {"enabled": True, "rotation_days": 90},
        }
        ...
        if api_config["api_key_rotation"]["rotation_days"] > MAX_API_KEY_ROTATION_DAYS:
