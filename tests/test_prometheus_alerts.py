...
def check_codex_metrics():
    ...
    try:
        ...
        response = requests.get(
            "http://localhost:9090/api/v1/query?query=codex_activity_seconds_bucket", timeout=5
        )
        if response.status_code == HTTP_OK:
            data = response.json()
    ...
    if "async def publish_portal(" in content:
        ...
        for line in lines:
            ...
            if "async def publish_portal(" in line:
                ...
                if not line.strip().startswith('"""') and not line.strip().startswith("'''"):
                    ...
                    new_lines.insert(
                        -1, "    time.sleep(65)  # Sleep for 65 seconds to exceed 60s threshold"
                    )
                    delay_added = True
                    in_publish_portal = False
        ...
        if delay_added:
            ...
            with open(activities_file, "w") as f:
                f.write("\n".join(new_lines))
            print("✅ Added 65-second delay to publish_portal function")
...
def main():
    ...
    if not check_alert_rules():
        ...
        print(
            "   Run: scripts/reload_prometheus.ps1 (Windows) or scripts/reload_prometheus.sh (Linux)"
        )
        return 1
