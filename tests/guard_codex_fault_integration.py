...
from fault_injection import FaultInjectionError, with_fault_injection
...
class ChaosTestRunner:
    ...
    async def run_duplicate_flood_test(
        self, base_delivery_id: str, count: int = 50
    ) -> dict[str, Any]:
        """
        ...
        for i in range(count):
            try:
                await self.service.handle_github_webhook(
                    delivery_id=base_delivery_id, event_type="pull_request", payload=test_payload
                )
                if i == 0:
    ...
    async def run_fault_injection_test(self, delivery_id: str) -> dict[str, Any]:
        ...
        try:
            await test_service.handle_github_webhook(
                delivery_id=delivery_id, event_type="pull_request", payload=test_payload
            )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print("===============")
        print(
            f"Duplicate Flood Test: {'✅ PASSED' if duplicate_results['test_passed'] else '❌ FAILED'}"
        )
        print(
            f"Fault Injection Test: {'✅ PASSED' if fault_results['test_passed'] else '❌ FAILED'}"
        )
