    ...
    try:
        ...
        if 'jobs' in workflow_data and 'build' in workflow_data['jobs']:
            ...
            if 'steps' in build_job:
                ...
                # Check step order
                build_idx = next((i for i, name in enumerate(step_names) if 'Build documentation' in name), -1)
                link_idx = next((i for i, name in enumerate(step_names) if 'HTML link check' in name), -1)
                deploy_idx = next((i for i, name in enumerate(step_names) if 'Deploy to GitHub Pages' in name), -1)
...
def main():
    ...
    print(f"   Link Check Scenarios: ✅ PASS")
