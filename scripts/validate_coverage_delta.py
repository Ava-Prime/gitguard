...
class CoverageDeltaValidator:
    ...
    def get_coverage_for_ref(self, git_ref: str) -> dict | None:
        """Get coverage data for a specific git reference."""

        try:
            # Checkout the reference
            subprocess.run(
                ["git", "checkout", git_ref], cwd=self.repo_root, check=True, capture_output=True
            )
    ...
    def calculate_delta(self, base_coverage: dict, head_coverage: dict) -> dict:
        ...
        for file_path in common_files:
            base_file_cov = (
                base_coverage["files"][file_path].get("summary", {}).get("percent_covered", 0)
            )
            head_file_cov = (
                head_coverage["files"][file_path].get("summary", {}).get("percent_covered", 0)
            )
    ...
    def validate_delta(self, delta: dict, threshold: float) -> dict:
        ...
        if passed:
            if total_delta > 0:
                validation["summary"] = (
                    f"✅ Coverage improved by {total_delta:.2f}% ({delta['base_coverage']:.1f}% → {delta['head_coverage']:.1f}%)"
                )
            else:
                validation["summary"] = (
                    f"✅ Coverage change ({total_delta:.2f}%) within acceptable threshold"
                )
        else:
            validation["summary"] = (
                f"❌ Coverage regression ({total_delta:.2f}%) exceeds threshold ({threshold:.2f}%)"
            )
...
def main():
    ...
    parser.add_argument("--base-ref", required=True, help="Base git reference for comparison")
    parser.add_argument("--head-ref", required=True, help="Head git reference for comparison")
    ...
    result = validator.run_validation(args.base_ref, args.head_ref, args.threshold)
