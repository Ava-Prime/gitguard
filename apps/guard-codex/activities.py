...
import os, json, subprocess, shutil, pathlib, datetime as dt, time
...
from ownership import parse_codeowners, owner_for, normalize_owner_handle, get_owner_type
...
# Prometheus metrics
DOC_FRESH = Histogram("codex_docs_freshness_seconds", "Event-to-doc write latency", buckets=(1,3,5,10,30,60,180,300,600))
...
def _validate_vector_dimension(vector: List[float], expected_dim: int = 1536) -> bool:
    """Validate vector dimension matches embedder expectation."""
    return len(vector) == expected_dim

def _cleanup_old_deliveries(days_to_keep: int = 90) -> None:
...
def _insert_embedding(cur, node_id: str, vector: List[float], model: str = 'text-embedding-3-large') -> None:
...
def _cleanup_temporary_branch_edges(days_to_keep: int = 90) -> None:
...
async def update_graph(facts: Dict[str, Any], analysis: Dict[str, Any]) -> None:
    ...
    with _conn() as c, c.cursor() as cur:
        ...
        if kind == "PR":
            ...
            repo_id = _upsert_node(cur, "Repo", facts["repo"], facts["repo_name"], {"full": facts["repo"]})
                ...
                if os.getenv("CODEX_OWNERS_ENABLED", "false").lower() == "true":
                    ...
                    if ownership:
                        ...
                        for handle in handles:
                            ...
                            owner_title = f"@{normalized_handle}" if owner_type == "user" else f"@{normalized_handle}"
                            owner_data = {
                                "handle": normalized_handle,
                                "type": owner_type,
                                "original_handle": handle
                            }
                            owner_id = _upsert_node(cur, "Owner", owner_key, owner_title, owner_data)
    ...
    with _conn() as c, c.cursor() as cur:
        ...
        if owners_file.exists():
            ...
            if table_start != -1:
                ...
                if table_end != -1:
                    table_end = content.find("*", table_end) + content[table_end:].find("\n") + table_end + 1
...
async def publish_portal(path: str) -> Dict[str, str]:
    ...
    repo_root = REPO_ROOT
    subprocess.check_call(["python", "-m", "mkdocs", "build", "--clean", "--site-dir", str(site)], cwd=str(repo_root))
