...
class MockGuardApiMetrics:
    ...
    def __init__(self):
        ...
        self.api_requests_total = MockMetric(
            "api_requests_total", "API requests", ["method", "endpoint", "status_code"]
        )
        self.api_request_duration_seconds = MockMetric(
            "api_request_duration_seconds", "API duration", ["method", "endpoint"]
        )
        self.webhook_events_total = MockMetric(
            "webhook_events_total", "Webhook events", ["event_type", "action", "status"]
        )
        self.webhook_processing_duration_seconds = MockMetric(
            "webhook_processing_duration_seconds", "Webhook duration", ["event_type", "action"]
        )
        self.webhook_signature_validations_total = MockMetric(
            "webhook_signature_validations_total", "Signature validations", ["status"]
        )
        self.active_connections = MockMetric(
            "active_connections", "Active connections", ["service"]
        )
        self.nats_messages_published_total = MockMetric(
            "nats_messages_published_total", "NATS messages", ["subject", "status"]
        )
        self.temporal_workflow_starts_total = MockMetric(
            "temporal_workflow_starts_total", "Workflow starts", ["workflow_type", "status"]
        )
        self.risk_score_calculations_total = MockMetric(
            "risk_score_calculations_total", "Risk calculations", ["result_category"]
        )
        self.risk_score_distribution = MockMetric("risk_score_distribution", "Risk distribution")
        self.codex_requests_total = MockMetric(
            "codex_requests_total", "Codex requests", ["endpoint", "status_code"]
        )
        self.codex_request_duration_seconds = MockMetric(
            "codex_request_duration_seconds", "Codex duration", ["endpoint"]
        )
...
class MockGuardCodexMetrics:
    ...
    def __init__(self):
        ...
        self.workflow_executions_total = MockMetric(
            "workflow_executions_total", "Workflow executions", ["workflow_type", "status"]
        )
        self.activity_executions_total = MockMetric(
            "activity_executions_total", "Activity executions", ["activity_name", "status"]
        )
        self.activity_failures_total = MockMetric(
            "activity_failures_total", "Activity failures", ["activity_name", "failure_type"]
        )
        self.workflow_failures_total = MockMetric(
            "workflow_failures_total", "Workflow failures", ["workflow_type", "failure_type"]
        )
        self.codex_events_processed_total = MockMetric(
            "codex_events_processed_total", "Codex events", ["event_type", "status"]
        )
        self.codex_docs_generated_total = MockMetric(
            "codex_docs_generated_total", "Docs generated", ["doc_type"]
        )
        self.codex_graph_updates_total = MockMetric(
            "codex_graph_updates_total", "Graph updates", ["update_type"]
        )
...
class TestGuardApiMetrics:
    ...
    def test_webhook_event_metrics(self):
        ...
        guard_api_metrics.record_webhook_event.assert_called_with(
            "pull_request", "opened", True, 0.8
        )
